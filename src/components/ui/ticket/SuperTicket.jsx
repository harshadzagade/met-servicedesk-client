...
const SuperTicket = ({type}) => {
    ...
    const updateCounts = (filteredData) => {
        ...
        setCountHodapproved(filteredData.filter(item =>  item.status === 'pending' && item.approval1 === 1 && (item.approval2 === 0 || item.approval2 === null)).length);
    };
                    ...
                    <Col xs={12} md={8}>
                        <List className={` m-3 ${classes.filters_wrap} `}>
                            ...
                            <ListInlineItem className={`${classes.filters} ${selectedStatus === 'pending' ? classes.active : ''}`} onClick={() => handleStatusSelect('pending')}>
                                Pending
                                {countPending > 0 && <span className={classes.notification}>{countPending-countHodapproved}</span>}
                            </ListInlineItem>
                            ...
                            {type !== 'Complaint' && (
                                <ListInlineItem className={`${classes.filters} ${selectedStatus === 'hod approved' ? classes.active : ''}`} onClick={() => handleStatusSelect('hod approved')}>
                                    HA
                                    {countHodapproved > 0 && <span className={classes.notification}>{countHodapproved}</span>}
                                </ListInlineItem>
                            )}
