...
const AllStaffList = () => {
    ...
    useEffect(() => {
        if (staffList.length !== 0 && (selectedData.length === staffList.length)) {
            setParentCheck('checked')
        }
    }, [selectedData, staffList]);
        ...
        <Fragment>
            {
                ...
                <Fragment>
                    ...
                    {showAlert && <div className="alert alert-danger" role="alert">Want to delete elements?&nbsp;<button className='btn btn-danger' onClick={handleMultipleDelete}>Delete</button></div>}
                    <div className={`${classes.tableParent}`}>
                        <table>
                            <thead>
                                <tr className={`${classes.tableHeadingRow}`}>
                                    <th scope="col"><input type="checkbox" checked={parentCheck} className={`${classes.parentCheckbox}`} onChange={handleParentCheckboxChange} /></th>
                            ...
                            <tbody>
                                {
                                    currentPageData.length > 0 && currentPageData.map((field) => (
                                        <tr className={`${classes.tableField}`} key={field.id}>
                                            <td data-label="checkbox"><input type="checkbox" className={`${classes.childCheckbox}`} id='childCheckbox' checked={childCheck} onChange={(e) => handleChildCheckboxChange(e, field)} />{/* <label htmlFor='childCheckbox' className={`${classes.childCheckboxStyling}`}>-</label> */}</td>
